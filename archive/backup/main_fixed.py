"""
Professional Invoice Manager v2.0 - Fixed Database Version
Enhanced version with external CSS, configuration, and comprehensive menu system
Fixed database column names and Qt constants
"""
...
class InvoiceListPage(QWidget):
    """Invoice list page with date column and fixed database queries"""
    ...
    def setup_table_keyboard_navigation(self):
        """Setup enhanced keyboard navigation for the table"""
        ...
        # Connect double-click and enter key to edit
    ...
    def eventFilter(self, obj, event):
        ...
        try:
            if obj == self.table and event.type() == event.KeyPress:
                ...
                # Enter/Return - Edit selected invoice
                ...
                # F10 - New invoice (legacy shortcut)
                elif key == 16777273:  # Qt.Key_F10
                    self.add_invoice()
                    return True
    ...
    def add_invoice(self):
        """Add new invoice - placeholder method"""
        parent = self.parent()
        if parent and hasattr(parent, 'status'):
            parent.status.showMessage("🔧 Új számla funkció fejlesztés alatt...", 3000)
    ...
    def edit_invoice(self):
        """Edit selected invoice - placeholder method"""
        if self.table.currentRow() < 0:
        ...
        parent = self.parent()
        if parent and hasattr(parent, 'status'):
            parent.status.showMessage("🔧 Számla szerkesztés funkció fejlesztés alatt...", 3000)
        QMessageBox.information(self, "Fejlesztés alatt", 
                               "A számla szerkesztés funkció jelenleg fejlesztés alatt áll.\n"
                               "Hamarosan elérhető lesz!")
    
    def delete_invoice(self):
        """Delete selected invoice - placeholder method"""
        if self.table.currentRow() < 0:
            QMessageBox.information(self, "Figyelem", "Kérem válasszon ki egy számlát!")
            return
        ...
        if reply == QMessageBox.Yes:
            parent = self.parent()
            if parent and hasattr(parent, 'status'):
                parent.status.showMessage("🔧 Számla törlés funkció fejlesztés alatt...", 3000)
            QMessageBox.information(self, "Fejlesztés alatt", 
...
class MainWindow(QMainWindow):
    """Enhanced main window with comprehensive features and fixed database"""
    
    def __init__(self):
        super().__init__()
        self.setup_ui()  # Setup UI first to create status bar
        self.init_database()
        self.setup_menus()
        self.load_styles()
        self.show_list()
    ...
    def setup_keyboard_navigation(self):
        ...
        edit_shortcut.activated.connect(self.edit_invoice)
        self.shortcuts.append(edit_shortcut)
        
        # Delete - Delete Invoice
        delete_shortcut = QShortcut(QKeySequence("Delete"), self)
        delete_shortcut.activated.connect(self.delete_invoice)
        self.shortcuts.append(delete_shortcut)
        
        # Insert - New Invoice (alternative)
        insert_shortcut = QShortcut(QKeySequence("Insert"), self)
        insert_shortcut.activated.connect(self.new_invoice)
        self.shortcuts.append(insert_shortcut)
        
        # Enter - Edit selected invoice
        enter_shortcut = QShortcut(QKeySequence("Return"), self)
        enter_shortcut.activated.connect(self.edit_selected_invoice)
        self.shortcuts.append(enter_shortcut)
        
        # Escape - Close current dialog or return to main view
        escape_shortcut = QShortcut(QKeySequence("Escape"), self)
        escape_shortcut.activated.connect(self.handle_escape)
        self.shortcuts.append(escape_shortcut)
        
        # Arrow navigation will be handled in table widgets directly
        
        # Update status bar with keyboard hints
        self.update_keyboard_hints()
    
    def update_keyboard_hints(self):
        """Update status bar with keyboard navigation hints"""
...
<tr><td><b>Ctrl+E</b></td><td>Számla szerkesztése</td></tr>
...
<p><i>💡 Tipp: Minden funkció elérhető billentyűzetről!</i></p>
    ...
    def new_invoice(self):
        """Create new invoice via keyboard shortcut"""
        if hasattr(self.list_page, 'add_invoice'):
            self.list_page.add_invoice()
            self.status.showMessage("📝 Új számla létrehozása...", 2000)
    
    def edit_invoice(self):
        """Edit invoice via keyboard shortcut"""
        if hasattr(self.list_page, 'edit_invoice'):
            self.list_page.edit_invoice()
            self.status.showMessage("✏️ Számla szerkesztése...", 2000)
    
    def delete_invoice(self):
        """Delete invoice via keyboard shortcut"""
        if hasattr(self.list_page, 'delete_invoice'):
            self.list_page.delete_invoice()
            self.status.showMessage("🗑️ Számla törlése...", 2000)
    
    def edit_selected_invoice(self):
        """Edit the currently selected invoice"""
        current_widget = self.stack.currentWidget()
        if hasattr(current_widget, 'table') and current_widget.table.currentRow() >= 0:
            self.edit_invoice()
        else:
            self.status.showMessage("❌ Nincs kiválasztva számla", 2000)
    
    def handle_escape(self):
        """Handle escape key - close dialogs or return to main view"""
        ...
        if active_modal:
            ...
            self.show_list()
            self.status.showMessage("🔙 Vissza a főoldalra", 2000)
    ...
    def manage_products(self):
        """Manage products with correct column names"""
        try:
            with get_db() as conn:
                ...
                # Show products list with correct column names
    ...
    def add_product(self):
        """Add new product with correct column names"""
        try:
    ...
    def add_partner(self):
        ...
        try:
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                with get_db() as conn:
                    conn.execute("""
                        INSERT INTO partner (name, kind, address, tax_id) 
                        VALUES (?, ?, ?, ?)
                    """, (
                        values["Név"],
    ...
    def about(self):
        ...
        QMessageBox.about(self, "📄 Számlázó Rendszer v2.0", 
                         ...
                         "• 🔧 Javított adatbázis kezelés<br><br>"
...
def main():
    ...
    try:
        ...
        window.status.showMessage("🎉 Számlázó Rendszer v2.0.1 - Fixed Database Edition elindítva")
